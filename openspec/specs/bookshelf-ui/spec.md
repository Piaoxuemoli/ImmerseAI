...
BookCard SHALL 渲染为 2:3 宽高比的卡片，包含封面区域和信息区域；**支持点击回调**。封面区域 SHALL 使用 `aspect-[2/3]` 实现固定比例。封面区域显示书名白色文字居中叠加在 muted 色调纯色背景上。封面下方显示书名（slate-900, font-semibold, text-sm）和作者名（slate-500, text-xs）。卡片 SHALL 有 rounded-lg 圆角和 slate-200 边框。
