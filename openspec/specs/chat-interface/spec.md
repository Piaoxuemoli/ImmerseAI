...
系统 SHALL 在 `src/features/chat/components/MessageBubble.tsx` 提供单条消息的展示组件，支持普通消息和笔记确认消息。
...
- **AND** 消息 `metadata.type` 不是 `'note-confirmation'`
...
#### Scenario: 笔记确认消息渲染
- **WHEN** 渲染 `role: 'assistant'` 的消息
- **AND** 消息 `metadata.type` 为 `'note-confirmation'`
- **THEN** 系统 SHALL 渲染 `NoteConfirmation` 组件替代普通 MessageBubble 内容
- **AND** NoteConfirmation SHALL 显示为绿色边框卡片（`border-green-200 bg-green-50`）
- **AND** 包含 📝 图标、笔记标题、文件路径
- **AND** 路径文本使用 `text-xs text-slate-500 font-mono` 样式

#### Scenario: 笔记错误消息渲染
- **WHEN** 渲染 `role: 'assistant'` 的消息
- **AND** 消息 `metadata.type` 为 `'note-error'`
- **THEN** 系统 SHALL 渲染为红色边框卡片（`border-red-200 bg-red-50`）
- **AND** 包含错误图标和错误原因文本
...
### Requirement: NoteConfirmation 笔记确认组件
系统 SHALL 在 `src/features/chat/components/NoteConfirmation.tsx` 提供笔记写入成功的确认展示组件。

#### Scenario: 成功确认渲染
- **WHEN** 渲染 NoteConfirmation 且 `success` 为 true
- **THEN** 组件 SHALL 显示：
  - 左侧 📝 图标（lucide-react `FileText`）
  - 标题文字 "笔记已保存"（`text-green-700 font-medium`）
  - 笔记标题（来自 `noteTitle` prop）
  - 文件路径（`font-mono text-xs text-slate-500`）
- **AND** 整体使用 `rounded-lg border border-green-200 bg-green-50 p-3` 样式
- **AND** 使用 framer-motion 入场动画（同 MessageBubble）

#### Scenario: 失败提示渲染
- **WHEN** 渲染 NoteConfirmation 且 `success` 为 false
- **THEN** 组件 SHALL 显示：
  - 左侧警告图标（lucide-react `AlertCircle`）
  - 标题文字 "笔记保存失败"（`text-red-700 font-medium`）
  - 错误原因（来自 `error` prop）
- **AND** 整体使用 `rounded-lg border border-red-200 bg-red-50 p-3` 样式

#### Scenario: 组件 Props 接口
- **WHEN** 使用 NoteConfirmation 组件
- **THEN** 组件 SHALL 接受以下 Props：
