...
### Requirement: 错误分级处理
系统 SHALL 对 LLM API 调用中的异常进行分类处理，区分初始化错误和流中错误。

#### Scenario: API Key 未配置
- **WHEN** `llm:chat` handler 被调用
- **AND** safeStorage 中未找到 API Key（空字符串）
- **THEN** handler 通过 Promise rejection 抛出错误
- **AND** 错误消息包含 `'API_KEY_NOT_CONFIGURED'` 标识

#### Scenario: 401 认证失败
- **WHEN** LLM API 返回 HTTP 401
- **THEN** 系统通过 `event.sender.send('llm:chat-error', { code: 'AUTH_FAILED', message })` 发送错误
- **AND** 随后发送 `'[DONE]'` 关闭流
...
#### Scenario: 网络超时
- **WHEN** LLM API 调用因网络问题超时
- **THEN** 系统通过 `event.sender.send('llm:chat-error', { code: 'NETWORK_ERROR', message })` 发送错误
- **AND** 随后发送 `'[DONE]'` 关闭流

#### Scenario: 通用异常
- **WHEN** 流式传输过程中发生未分类的异常
- **THEN** 系统通过 `event.sender.send('llm:chat-error', { code: 'UNKNOWN_ERROR', message: String(error) })` 发送错误
- **AND** 随后发送 `'[DONE]'` 关闭流
