...
- **AND** 将返回的 BookFile[] 过滤出 .epub（或 path 以 .epub 结尾），转换为 Book[]（见「Book 转换规则」）
...
- **AND** 按相同规则过滤并转换为 Book[]，调用 `setBooks(books)`
...
- **THEN** 仅处理 `type === 'epub'` 或 path 以 `.epub` 结尾的条目
- **AND** 每个 Book 的 `id` SHALL 为 `crypto.randomUUID()` 生成的新 UUID
- **AND** `title` SHALL 为文件名去掉 `.epub` 后缀（如 `三体.epub` → `三体`）
