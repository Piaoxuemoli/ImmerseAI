    ...
    <div className="flex flex-col divide-y divide-border px-6 py-2">
      {books.map((book, index) => {
        ...
        return (
          <button
            key={book.id}
            className="flex items-center gap-3 py-2.5 text-left hover:bg-muted rounded-md px-2 -mx-2 transition-colors"
          ...
          >
            ...
            {/* 上次阅读时间 */}
            {lastRead && (
              <span className="shrink-0 text-xs text-muted-foreground">{lastRead}</span>
            )}
          </button>
        )
      })}
